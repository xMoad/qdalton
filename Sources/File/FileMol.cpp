...
bool File::Mol::generate()
{
  ...
  QList<Render::Atom> obAtomsList;

  for (quint16 i = 0; i < molecule().atomsCount(); ++i)
  {
    obAtomsList << molecule().atom(i);
  }

  while (obAtomsList.count() > 0)
  {
    QList<Render::Atom> obAtomsOfTheSameTypeList;
    obAtomsOfTheSameTypeList << obAtomsList.takeFirst();
    quint8 charge = obAtomsOfTheSameTypeList[0].atomicNumber();
    int n = obAtomsList.count();
    int i = 0;
    while (i < n)
    {
      if (obAtomsList[i].atomicNumber() == charge)
      {
    ...
    QString atomsOfTheSameTypeHeader = QString("Charge=%1 Atoms=%2").arg(
        charge).arg(obAtomsOfTheSameTypeList.count());
    ...
    {
      x = obAtomsOfTheSameTypeList[i].centre().x();
      y = obAtomsOfTheSameTypeList[i].centre().y();
      z = obAtomsOfTheSameTypeList[i].centre().z();
      ...
      QString temp = QString("%1%2\t%3%4%5").arg(
          obAtomsOfTheSameTypeList[i].symbol()).arg(
              i + 1).arg(
                  x, 12, 'f', 6).arg(
                      y, 12, 'f', 6).arg(
                          z, 12, 'f', 6);

      if (obAtomsOfTheSameTypeList[i].isotope() != 0)
        temp += QString("\tIsotope=%1").arg(obAtomsOfTheSameTypeList[i].isotope());
    ...
    if (regExp.indexIn(string(i)) != -1)
    {
      ...
      for (int j = 0; j < count; j++)
      {
        ...
        if (regExpAtom.indexIn(string(i)) != -1)
        {
          ...
          atom.setCentre(Eigen::Vector3f(x, y, z));
